  ...
  var UI = (() => {
    ...
    async function shareLink() {
      ...
      try {
        ...
        simStatusToast("Share link written to URL \u2014 copy from address bar.", 4e3);
      }
    }
...
${liveTitle}` : liveTitle;
    ...
    function syncFileControlAffordances(sourceUi = readSourceUiModel()) {
      ...
      syncControlCopy(
        ui.btnLoad,
        fileControlsDisabled ? readFileModeOnlyAffordanceText("Load") : "Load audio files into the queue"
      );
      syncControlCopy(
        ui.btnPlay,
      ...
      syncControlCopy(
        ui.btnPrev,
        fileControlsDisabled ? readFileModeOnlyAffordanceText("Previous track") : ui.btnPrev && ui.btnPrev.disabled ? "Previous track unavailable" : "Previous track (P)"
      );
      syncControlCopy(
        ui.btnNext,
        fileControlsDisabled ? readFileModeOnlyAffordanceText("Next track") : ui.btnNext && ui.btnNext.disabled ? "Next track unavailable" : "Next track (N)"
      );
      syncControlCopy(
        ui.btnRepeat,
      ...
      syncControlCopy(
        ui.btnClearQueue,
        fileControlsDisabled ? readFileModeOnlyAffordanceText("Clear queue") : "Clear file queue"
      );
    }
    ...
    function refreshAllUiText(bandSnapshot) {
      ...
      ui.valOmega.textContent = `${fmt(p.motion.angularSpeedRadPerSec, 3)} rad/s (${fmt(p.motion.angularSpeedRadPerSec * RAD_TO_DEG, 1)}\xB0/s)`;
      ...
      ui.valHueOff.textContent = `${p.bands.rainbow.hueOffsetDeg}\xB0`;

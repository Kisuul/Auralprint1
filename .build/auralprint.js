(() => {
  ...
  var UI = (() => {
    ...
    function refreshBandHud() {
      ...
      span.textContent = `Dominant [${domIdx}] ${domName} \xE2\u20AC\u201D ${domRange}`;
    ...
    function refreshBandMetaText() {
      ...
      const sampleRateText = Number.isFinite(m.sampleRateHz) ? formatBandMetaHz(m.sampleRateHz) : "pending audio context";
      ui.bandMeta.textContent = `${bandCount} bands \xE2\u20AC\xA2 Nyquist ${formatBandMetaHz(m.nyquistHz)} \xE2\u20AC\xA2 ceiling configured ${formatBandMetaHz(m.configCeilingHz)}`;
    }
